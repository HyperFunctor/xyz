...
export function PostForm({ currentUser }: { currentUser: { id: number; username: string } }) {
	async function postMessage(form: FormData) {
		"use server";
		...
		await db.insert(tweet).values([input]);

		revalidatePath(`/${currentUser.username}`);
	}

	return (
		<div className="dark:border-dim-200 border-b border-l border-r border-gray-200 pb-4">
			<Form action={postMessage}>
				<div className="flex flex-shrink-0 p-4 pb-0">
					<div className="items-top flex w-12">
						<Image
							className="inline-block h-10 w-10 rounded-full"
							src={`https://api.dicebear.com/7.x/adventurer/png?seed=${currentUser.username}&flip=true`}
							alt="avatar"
							width={128}
							height={128}
						/>
				...
				<div className="items-top flex w-full p-2 pl-14 text-white">
					<a href="#" className="dark:hover:bg-dim-800 rounded-full p-2 text-rose-400 hover:bg-rose-50">
					...
					<button
						type="submit"
						className="ml-auto mr-1 rounded-full bg-rose-400 px-4 py-1 text-white hover:bg-rose-500"
					>
